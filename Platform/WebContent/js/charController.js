...
function checkMovement() {
	if (controllingChar) {
		if (keyMap[87]) { // W
			if (keyMap[16]
				...
				charMesh.applyCentralForce(new THREE.Vector3(
						finalForceVector.x, 0, finalForceVector.z));
				stamina -= 2;
		...
		if (intersects[0]) {
			if (charMesh.getLinearVelocity().y < -14
					&& intersects[0].distance < 2) {
	...
	// camera.lookAt(charMesh.position); //This doesnt turn out exactly how it
